...
export default defineConfig(({ mode }) => {
  ...
  // Docker / low-RAM hosts: set VITE_NO_SOURCEMAP=1 and VITE_MINIFY=esbuild (see Dockerfiles)
  const useEsbuildMinify =
    process.env.VITE_MINIFY === 'esbuild' ||
    (process.env.VITE_NO_SOURCEMAP === '1' &&
  ...
  return {
    ...
    build: {
      ...
      minify: (useEsbuildMinify ? 'esbuild' : 'terser') as 'esbuild' | 'terser',
      terserOptions: {
        compress: {
      ...
      sourcemap: process.env.VITE_NO_SOURCEMAP === '1' ? false : true,
      cssCodeSplit: true,
      target: 'es2015',
    },
    esbuild: {
