...
示例：general/v2/`,
      ...
      mineru: {
        ...
        accessToken: '访问 Token（可选）',
        accessTokenPlaceholder:
          '如阿里云 PAI-EAS 推理 Token，填入后将置于 Authorization 请求头',
...
[在此处插入文本]`,
      ...
      grader: 'Grader',
      graderDescription: '评估检索结果，过滤不相关文档，提高答案质量。',
      graderEvalMode: '评估模式',
      graderEvalModeTip: '选择评估方法：LLM-as-Judge（语义相关性判断，精度最高）或 NLI（自然语言推理，适合事实性查询）。当评估失败时自动降级为 Rerank 分数。',
      graderBatchSize: '批量大小',
      graderBatchSizeTip: '每批评估的文档数量。',
      graderRelevanceThreshold: '相关性阈值',
      graderRelevanceThresholdTip: '文档被视为相关的最小相关性分数（0-1）。',
      graderMaxEvalTokens: '最大评估 Token 数',
      graderMaxEvalTokensTip: '每次评估请求允许的最大 token 数。',
      graderTimeout: '超时时间',
      graderTimeoutTip: '评估超时时间（秒）。',
      hallucinationDetector: '幻觉检测器',
      hallucinationDetectorDescription: '根据检索文档验证答案的忠实度，检测并处理幻觉。',
      hallucinationDetectorPassThreshold: '通过阈值',
      hallucinationDetectorPassThresholdTip: '通过验证的忠实度分数阈值（0-1）。',
      hallucinationDetectorFilterThreshold: '过滤阈值',
      hallucinationDetectorFilterThresholdTip: '触发过滤的忠实度分数阈值（0-1）。',
      hallucinationDetectorRegenerateThreshold: '重新生成阈值',
      hallucinationDetectorRegenerateThresholdTip: '触发重新生成的忠实度分数阈值（0-1）。',
      hallucinationDetectorRuleWeight: '规则权重',
      hallucinationDetectorRuleWeightTip: '基于规则的验证层权重（0-1）。',
      hallucinationDetectorNliWeight: 'NLI 权重',
      hallucinationDetectorNliWeightTip: '基于 NLI 的验证层权重（0-1）。',
      hallucinationDetectorLlmWeight: 'LLM 权重',
      hallucinationDetectorLlmWeightTip: '基于 LLM 的验证层权重（0-1）。',
      hallucinationDetectorTimeout: '超时时间',
      hallucinationDetectorTimeoutTip: '检测超时时间（秒）。',
      queryRewriter: '查询重写器',
      queryRewriterDescription: '分析查询复杂度并应用重写策略，提高检索召回率和精确度。',
      queryRewriterStrategy: '策略',
      queryRewriterStrategyTip: '选择重写策略：同义词扩展、子查询分解或 HyDE。',
      queryRewriterUseLlmForSynonyms: '使用 LLM 获取同义词',
      queryRewriterUseLlmForSynonymsTip: '使用 LLM 将同义词自然地融入查询。',
      subQueryDecomposer: '子查询分解器',
      subQueryDecomposerDescription: '将复杂查询分解为多个子查询，并使用 RRF 算法合并结果。',
      subQueryDecomposerMinCount: '最小数量',
      subQueryDecomposerMinCountTip: '生成的子查询最小数量。',
      subQueryDecomposerMaxCount: '最大数量',
      subQueryDecomposerMaxCountTip: '生成的子查询最大数量。',
      subQueryDecomposerTopK: 'Top K',
      subQueryDecomposerTopKTip: '每个子查询检索的结果数量。',
      subQueryDecomposerDedupThreshold: '去重阈值',
      subQueryDecomposerDedupThresholdTip: '语义去重的相似度阈值（0-1）。',
      subQueryDecomposerRrfK: 'RRF K',
      subQueryDecomposerRrfKTip: '倒数秩融合算法中的常数 k。',
      hyde: 'HyDE',
      hydeDescription: '生成假设文档嵌入，提高事实性查询的检索效果。',
      hydeEnableHyde: '启用 HyDE',
      hydeEnableHydeTip: '启用假设文档嵌入生成。',
      hydePrompt: 'HyDE 提示词',
      hydePromptTip: '生成假设文档的提示词模板。',
      hydeTemperature: '温度',
      hydeTemperatureTip: '生成的采样温度（0-2）。',
      hydeMaxTokens: '最大 Token 数',
      hydeMaxTokensTip: '生成文档的最大 token 数。',
      retryController: '重试控制器',
      retryControllerDescription: '根据检索质量、重试次数和 token 成本控制重试逻辑。',
      retryControllerMaxRetries: '最大重试次数',
      retryControllerMaxRetriesTip: '最大重试尝试次数。',
      retryControllerMaxRetryTokens: '最大重试 Token 数',
      retryControllerMaxRetryTokensTip: '重试允许的最大累积 token 数。',
      retryControllerMinRelevantDocs: '最小相关文档数',
      retryControllerMinRelevantDocsTip: '停止重试所需的最小相关文档数量。',
      retryControllerHasRelevant: '是否有相关文档',
      retryControllerHasRelevantTip: 'has_relevant 状态的变量引用。',
      retryControllerRelevantCount: '相关文档数量',
      retryControllerRelevantCountTip: '相关文档数量的变量引用。',
      retryControllerRetryCount: '重试次数',
      retryControllerRetryCountTip: '当前重试次数的变量引用。',
      retryControllerAccumulatedRetryTokens: '累积重试 Token 数',
      retryControllerAccumulatedRetryTokensTip: '累积重试 token 数的变量引用。',
    },
    footer: {
