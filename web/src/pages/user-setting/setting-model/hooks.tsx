...
export const useSubmitMinerU = () => {
  ...
  const onMineruOk = useCallback(
    async (payload: MinerUFormValues, isVerify = false) => {
      ...
      const raw = payload as MinerUFormValues & { verify?: boolean };
      const { verify: _v, ...rest } = raw;
      const cfg: any = {
        ...rest,
      ...
      const mineruToken =
        typeof cfg.mineru_token === 'string' ? cfg.mineru_token.trim() : '';
      ...
      const ret = await addLlm({
        ...req,
        verify: isVerify,
        ...(mineruToken ? { mineru_token: mineruToken } : {}),
      });
      if (!isVerify) {
        setSaveLoading(false);
        if (ret.code === 0) {
