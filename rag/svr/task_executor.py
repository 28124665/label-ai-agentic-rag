...
from common.constants import LLMType, ParserType, PipelineTaskType, MessageTypeEnum
...
from common.versions import get_data_knowledge_api_version
...
async def build_chunks(task, progress_callback):
    ...
    try:
        ...
        logging.info("From minio({}) {}".format(timer() - st, task["location"]))
    except TimeoutError:
    ...
    doc = {
        ...
        "location": task["location"],  # 添加 location 字段
        "message_type": str(task["message_type"])  # 添加 message_type 字段
    }
    if task["pagerank"]:
...
def init_kb(row, vector_size: int):
    ...
    # 检查是否有message_type且在MessageTypeEnum中
    message_type = row.get("message_type")

    if message_type and message_type in MessageTypeEnum and not idxnm.startswith("ragflow_doc_meta_"):
        idxnm = f"{message_type}:{idxnm}"
    parser_id = row.get("parser_id", None)

    logging.info(f"init_kb.message_type ({message_type}), idxnm ({idxnm})")
...
async def insert_chunks(task_id, task_tenant_id, task_dataset_id, chunks, progress_callback):
    ...
    for ck in chunks:
        ...
        if chunk_id and chunk_id not in chunk_ids:
            ...
            # 如果没有 ID，生成一个新的
            new_id = xxhash.xxh64((ck["content_with_weight"] + str(ck["doc_id"]) + str(datetime.now().timestamp())).encode("utf-8", "surrogatepass")).hexdigest()
            ck["id"] = new_id
            chunk_ids.add(new_id)
            unique_chunks.append(ck)
    chunks = unique_chunks

    # 获取索引名
    idxnm = search.index_name(task_tenant_id)
    # 从第一个chunk中获取message_type（如果有）
    if chunks:
        message_type = chunks[0].get("message_type")
        if message_type and message_type in MessageTypeEnum and not idxnm.startswith("ragflow_doc_meta_"):
            idxnm = f"{message_type}:{idxnm}"
...
async def do_handle_task(task):
    ...
    logging.info(f"1.do_handle_task, task_type: {task_type}, task_id: {task['id']}")
    ...
    try:
        ...
        logging.info(f"2.do_handle_task, embedding_model.vector_size: {vector_size}, task_id: {task_id}")
    except Exception as e:
    ...
    logging.info(f"3.do_handle_task.init_kb.end, task_id: {task_id}")
    ...
    # Either using graphrag or Standard chunking methods
        ...
        logging.info("4.do_handle_task.build_chunks={}: {:.2f}s".format(task_document_name, timer() - start_ts))
        if not chunks:
        ...
        try:
            ...
            token_count = 0
            raise
        progress_message = "5.do_handle_task.Embedding chunks ({:.2f}s)".format(timer() - start_ts)
    ...
    async def _maybe_insert_chunks(_chunks):
        ...
        logging.info(f"6.do_handle_task.insert_chunks={chunk_count}: {timer() - start_ts:.2f}s")
    ...
    try:
        ...
        logging.info(
            "7.do_handle_task.Indexing doc({}), page({}-{}), chunks({}), elapsed: {:.2f}".format(
        ...
        logging.info(f"8.do_handle_task.Indexing doc({task_document_name}), page({task_from_page}-{task_to_page}), chunks({chunk_count}), token({token_count}), elapsed: {timer() - start_ts:.2f}s")
        ...
        logging.info(
            "9.do_handle_task.Chunk doc({}), page({}-{}), chunks({}), token({}), elapsed:{:.2f}".format(
        ...
        if has_canceled(task_id):
            try:
                # 获取索引名，包含message_type
                idxnm = search.index_name(task_tenant_id)
                message_type = task.get("message_type")
                if message_type and message_type in MessageTypeEnum and not idxnm.startswith(
                        "ragflow_doc_meta_"):
                    idxnm = f"{message_type}:{idxnm}"
...
/___/_/ /_/\__, /\___/____/\__/_/\____/_/ /_/  /____/\___/_/    |___/\___/_/
          /____/
    """)
    logging.info(f'data-knowledge-api version: {get_data_knowledge_api_version()}')
    show_configs()
