...
from data_knowledge_api_client import DataKnowledgeAPIClient, run_command
...
class DataKnowledgeAPICLI(Cmd):
    ...
    def verify_auth(self, arguments: dict, single_command: bool, auth: bool):
        ...
        if not auth:
            self.data_knowledge_api_client = DataKnowledgeAPIClient(http_client, server_type)
            return True
    ...
    def execute_command(self, parsed_command: Dict[str, Any]):
        ...
        run_command(self.data_knowledge_api_client, command_dict)

def main():

    cli = DataKnowledgeAPICLI()
