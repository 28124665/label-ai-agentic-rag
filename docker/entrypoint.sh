...
function usage() {
    ...
    echo
    echo "  --disable-webserver             Disables the web server (nginx + data-knowledge-api_server)."
...
MCP_SCRIPT_PATH="/data-knowledge-api/mcp/server/server.py"
MCP_MODE="self-host"
MCP_HOST_API_KEY=""
...
# -----------------------------------------------------------------------------
CONF_DIR="/data-knowledge-api/conf"
...
if [[ "${ENABLE_WEBSERVER}" -eq 1 ]]; then
    echo "Starting nginx..."
    # Stock nginx package ships default.conf (welcome page, root /usr/share/nginx/html).
    # Remove it so only conf.d/data-knowledge-api.conf (image or compose mount) serves :80.
    rm -f /etc/nginx/conf.d/default.conf
    /usr/sbin/nginx

    echo "Starting data-knowledge-api_server..."
    # Python API on :9380 (nginx proxies here). bin/server_main is optional Go binary from build.sh — not shipped in Docker image.
    while true; do
        "$PY" api/data-knowledge-api_server.py ${INIT_SUPERUSER_ARGS} &
        wait;
        sleep 1;
    done &
fi
