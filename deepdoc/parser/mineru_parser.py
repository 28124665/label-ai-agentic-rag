...
class MinerUParser(RAGFlowPdfParser):
    ...
    def _http_headers(self, base: Optional[dict] = None) -> dict[str, str]:
        headers = dict(base) if base else {}
        headers.setdefault("Accept", "application/json")
        if self.mineru_token:
            headers["Authorization"] = self.mineru_token
        return headers

    def _auth_header_candidates(self) -> list[dict[str, str]]:
        """Generate compatible Authorization header variants."""
        base = {"Accept": "application/json"}
        token = (self.mineru_token or "").strip()
        if not token:
            return [base]

        candidates: list[dict[str, str]] = []

        # Variant 1: use as-is (PAI-EAS commonly expects this)
        candidates.append({**base, "Authorization": token})

        # Variant 2: auto-add Bearer when token has no scheme prefix
        lowered = token.lower()
        if not any(lowered.startswith(prefix) for prefix in ("bearer ", "token ", "apikey ", "api-key ")):
            candidates.append({**base, "Authorization": f"Bearer {token}"})
    ...
    @staticmethod
    def _is_http_endpoint_valid(url, timeout=5, headers=None):
        hdrs = headers or {}
        try:
            response = requests.head(url, timeout=timeout, allow_redirects=True, headers=hdrs)
            if response.status_code == 405:
                response = requests.get(url, timeout=timeout, allow_redirects=True, headers=hdrs, stream=True)
                try:
                    response.close()
                except Exception:
                    pass
    ...
    @staticmethod
    def _probe_http_endpoint(url: str, timeout=8, headers=None) -> tuple[bool, int, str]:
        """Probe endpoint with HEAD, fallback to GET when needed."""
        hdrs = headers or {}
        try:
            response = requests.head(url, timeout=timeout, allow_redirects=True, headers=hdrs)
            ...
            if status_code == 405:
                ...
                try:
                    ...
                    pass
            return status_code in [200, 301, 302, 307, 308], status_code, response.url
        except Exception:
            return False, -1, url
    ...
    def check_installation(self, backend: str = "pipeline", server_url: Optional[str] = None) -> tuple[bool, str]:
        ...
        api_docs = f"{self.mineru_api}/docs"
        api_root = self.mineru_api
        try:
            checks = [
                ("openapi.json", api_openapi),
                ("docs", api_docs),
                ("root", api_root),
            ]
            last_status = -1
            ...
            auth_failed = False
            for label, check_url in checks:
                for idx, probe_headers in enumerate(self._auth_header_candidates()):
                    api_ok, status_code, final_url = self._probe_http_endpoint(check_url, headers=probe_headers)
                    last_status = status_code
                    auth_mode = "as-is" if idx == 0 else "bearer"
                    self.logger.info(
                        f"[MinerU] probe {label}({auth_mode}): reachable={api_ok} status={status_code} "
                        f"url={check_url} final_url={final_url}"
                    )
                    if api_ok:
            ...
            if not probe_succeeded:
                if auth_failed:
                    reason = (
                        "[MinerU] Authorization failed when probing MinerU API. "
                        "Please verify token value and Authorization header format."
                    )
                    return False, reason
                reason = f"[MinerU] MinerU API not accessible: {api_openapi} (last_status={last_status})"
                return False, reason
    ...
    def _run_mineru_api(
        ...
        try:
            ...
            for idx, headers in enumerate(self._auth_header_candidates()):
                auth_mode = "as-is" if idx == 0 else "bearer"
                self.logger.info(f"[MinerU] try file_parse with auth mode: {auth_mode}")
                with open(pdf_file_path, "rb") as pdf_file:
                    ...
                    try:
                        with requests.post(
                            ...
                            if response.status_code in (401, 403) and idx + 1 < len(self._auth_header_candidates()):
                                self.logger.warning(
                                    f"[MinerU] file_parse auth failed with mode {auth_mode}, status={response.status_code}; retrying..."
                                )
                                continue

                            if not response.ok:
                                try:
                                    err_body = (response.text or "")[:4096]
                                except Exception:
                                    err_body = ""
                                self.logger.error(
                                    f"[MinerU] file_parse HTTP {response.status_code} url={self.mineru_api}/file_parse "
                                    f"auth={auth_mode} body_preview={err_body!r}"
                                )
                        ...
                        last_error = req_err
                        if idx + 1 < len(self._auth_header_candidates()):
                            self.logger.warning(f"[MinerU] file_parse failed with mode {auth_mode}: {req_err}; retrying...")
                            continue
                        raise
            ...
            if not response_obtained:
                if last_error:
                    raise last_error
                raise RuntimeError("[MinerU] file_parse failed without response.")
