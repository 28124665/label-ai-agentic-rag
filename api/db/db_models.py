...
class Document(DataBaseModel):
    ...
    status = CharField(max_length=1, null=True, help_text="is it validate(0: wasted, 1: validate)", default="1", index=True)
    message_type = CharField(max_length=255, null=True, help_text="message type", index=True)
...
def alter_db_add_column(migrator, table_name, column_name, column_type):
    try:
        ...
        error_messages = ['already exists']
        if any(msg in str(ex) for msg in error_messages):
            logging.warning(
                f"Failed to add {settings.DATABASE_TYPE.upper()}.{table_name} column {column_name}, error: {ex}")
        else:
            raise
...
def update_tenant_llm_to_id_primary_key():
    ...
    try:
        with DB.atomic():
            ...
            # 2. Set ID - Database specific
            if settings.DATABASE_TYPE == "postgres":
                DB.execute_sql("""
                    UPDATE tenant_llm 
                    SET temp_id = row_number() OVER (ORDER BY tenant_id, llm_factory, llm_name)
                """)
            else:
            ...
            # 4. Update ID column to primary key - Database specific
            if settings.DATABASE_TYPE == "postgres":
                DB.execute_sql("""
                ...
                """)
                DB.execute_sql("""
                CREATE SEQUENCE tenant_llm_temp_id_seq;
                """)
                DB.execute_sql("""
                ALTER TABLE tenant_llm 
                ALTER COLUMN temp_id SET DEFAULT nextval('tenant_llm_temp_id_seq');
                """)
                DB.execute_sql("""
                ALTER TABLE tenant_llm 
