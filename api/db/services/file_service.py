...
class FileService(CommonService):
    ...
    def upload_document(self, kb, file_objs, user_id, src="local", parent_path: str | None = None):
        ...
        logging.info(f"1.file_service.upload_document kb({kb}), file_objs({file_objs}), user_id({user_id}), src({src}), parent_path({parent_path})")
        ...
        for file in file_objs:
            ...
            try:
                ...
                while settings.STORAGE_IMPL.obj_exist(kb.id, location):
                    location += "_"
                logging.info(f"2.file_service.upload_document filename({filename}), location({location})")

                blob = file.read()
                if filetype == FileType.PDF.value:
                    blob = read_potential_broken_pdf(blob)
                settings.STORAGE_IMPL.put(kb.id, location, blob)
                logging.info(f"3.file_service.settings.STORAGE_IMPL.put filetype({filetype}), location({location})")
                ...
                logging.info(f"4.file_service.upload_document.DocumentService.insert doc.id({doc['id']}), kb_folder.id({kb_folder['id']})")

                FileService.add_file_from_kb(doc, kb_folder["id"], kb.tenant_id)
                logging.info(f"5.file_service.upload_document.FileService.add_file_from_kb doc.id({doc['id']}), kb_folder.id({kb_folder['id']})")
    ...
    @classmethod
    @DB.connection_context()
    def upload_oss_document(cls, kb, source_oss_url, user_id, message_type=None):
        from rag.utils.oss_conn import RAGFlowOSS
        logging.info(f"1.file_service.upload_oss_document kb({kb}), source_oss_url({source_oss_url}), user_id({user_id}), message_type({message_type})")
        ...
        try:
            ...
            if not bucket or not key:
                err.append(f"Invalid OSS URL: {source_oss_url}")
                return err, files
            logging.info(f"2.file_service.upload_oss_document bucket({bucket}), key({key})")

            # 文件判重：根据 tenant_id 和 location 查询
            location = key
            existing_docs = DocumentService.query(tenant_id=kb.tenant_id, location=location)
            if existing_docs:
                # 文件已存在，不再处理
                logging.info(f"3.file_service.upload_oss_document existing_docs({existing_docs})")
                err.append(f"upload_oss_document.existing_docs: {source_oss_url}")
                return err, files

            # 从 OSS 下载文件
            blob = oss_client.get_no_prefix_path(bucket, key)
            if not blob:
                err.append(f"Failed to download file from OSS: {source_oss_url}")
                return err, files

            # 生成文件名
            filename = key.split("/")[-1]
            if not filename:
                err.append(f"Invalid file path in OSS URL: {source_oss_url}")
                return err, files
            logging.info(f"4.file_service.upload_oss_document filename({filename})")

            # 检查文件类型
            filetype = filename_type(filename)
            logging.info(f"5.file_service.upload_oss_document filetype({filetype})")
            ...
            DocumentService.insert(doc)
            logging.info(f"6.file_service.upload_oss_document doc.id({doc['id']}), doc.location({doc['location']})")

            # 添加到文件系统
            FileService.add_file_from_kb(doc, kb_folder["id"], kb.tenant_id)
            logging.info(f"7.file_service.upload_oss_document doc.id({doc['id']}), kb_folder.id({kb_folder['id']})")

            files.append((doc, blob))
        except Exception as e:
            err.append(f"Error processing OSS file: {str(e)}")
