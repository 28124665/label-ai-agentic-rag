...
def _probe_timeout_seconds(*, verify: bool) -> int:
    """Probe timeout for set_api_key / add_llm (embedding, chat stream, rerank, etc.).
    Default 200s for both verify and save-key paths; override with LLM_VERIFY_TIMEOUT_SECONDS / LLM_TIMEOUT_SECONDS."""
    if verify:
        return int(os.environ.get("LLM_VERIFY_TIMEOUT_SECONDS", "200"))
    return int(os.environ.get("LLM_TIMEOUT_SECONDS", "200"))


def _probe_stream_chunk_ok(chunk) -> bool:
    """async_chat_streamly yields text deltas then a final int (total_tokens); ignore empty / errors."""
...
async def set_api_key():
    ...
    verify_flag = req.get("verify", False)
    timeout_seconds = _probe_timeout_seconds(verify=verify_flag)
    logging.info(
        "[set_api_key] start factory=%s source_fid=%s verify=%s timeout_s=%s base_url=%r elapsed_since_accept=%.3fs",
        factory,
        source_factory,
        verify_flag,
        timeout_seconds,
    ...
    if not source_llms:
        ...
        logging.warning("[set_api_key] no LLM rows for fid=%s", source_factory)
        if verify_flag:
            return get_json_result(data={"message": msg, "success": False})
        return get_data_error_result(message=msg)

    probe_order = [(x.llm_name, x.model_type, x.fid) for x in source_llms]
    logging.info("[set_api_key] catalog_models count=%s order=%s", len(source_llms), probe_order)
    ...
    for llm in source_llms:
        ...
        if not embd_passed and llm.model_type == LLMType.EMBEDDING.value:
            ...
            logging.info(
                "[set_api_key] probe EMBEDDING llm_name=%s fid=%s",
                llm.llm_name,
                llm.fid,
            )
            ...
            try:
                ...
                embd_passed = True
                logging.info(
                    "[set_api_key] embedding OK llm_name=%s tokens=%s elapsed=%.3fs",
                    llm.llm_name,
                    tc,
                ...
                logging.exception(
                    "[set_api_key] embedding FAIL llm_name=%s elapsed=%.3fs",
                    llm.llm_name,
                    time.perf_counter() - t_probe,
                )
            ...
            logging.info(
                "[set_api_key] probe CHAT llm_name=%s fid=%s timeout_s=%s",
                llm.llm_name,
                llm.fid,
                timeout_seconds,
            )
            ...
            try:
                async def check_streamly():
                    ...
                    async for chunk in mdl.async_chat_streamly(
                        ...
                        chunk_idx += 1
                        ok = _probe_stream_chunk_ok(chunk)
                        if chunk_idx <= 8 or ok:
                            prev = chunk
                            if isinstance(chunk, str):
                                prev = chunk[:160] + ("…" if len(chunk) > 160 else "")
                            logging.info(
                                "[set_api_key] stream chunk i=%s type=%s probe_ok=%s preview=%r elapsed=%.3fs model=%s",
                                chunk_idx,
                                type(chunk).__name__,
                                ok,
                                prev,
                        ...
                        if ok:
                            logging.info(
                                "[set_api_key] stream first_valid i=%s elapsed=%.3fs model=%s",
                                chunk_idx,
                                time.perf_counter() - t_stream,
                                llm.llm_name,
                            )
                            return True
                    logging.warning(
                        "[set_api_key] stream end no_valid_chunk total_chunks=%s elapsed=%.3fs model=%s",
                        chunk_idx,
                        time.perf_counter() - t_stream,
                        llm.llm_name,
                    )
                ...
                try:
                    ...
                    logging.error(
                        "[set_api_key] stream TIMEOUT after %ss model=%s (no valid chunk in window)",
                        timeout_seconds,
                        llm.llm_name,
                    )
                    raise
                if result:
                    chat_passed = True
                    logging.info(
                        "[set_api_key] chat OK llm_name=%s probe_elapsed=%.3fs",
                        llm.llm_name,
                        time.perf_counter() - t_probe,
                    )
                else:
                ...
                logging.exception(
                    "[set_api_key] chat FAIL llm_name=%s elapsed=%.3fs err=%s",
                    llm.llm_name,
                    time.perf_counter() - t_probe,
                    e,
                )
            ...
            logging.info("[set_api_key] probe RERANK llm_name=%s fid=%s", llm.llm_name, llm.fid)
            ...
            try:
                ...
                rerank_passed = True
                logging.info(
                    "[set_api_key] rerank OK llm_name=%s elapsed=%.3fs",
                    llm.llm_name,
                    time.perf_counter() - t_probe,
                )
            except Exception as e:
                logging.exception(
                    "[set_api_key] rerank FAIL llm_name=%s elapsed=%.3fs",
                    llm.llm_name,
                    time.perf_counter() - t_probe,
                )
                msg += f"\nFail to access model({llm.fid}/{llm.llm_name}) using this api key." + str(e)
        if any([embd_passed, chat_passed, rerank_passed]):
            msg = ""
            logging.info(
                "[set_api_key] probe short_circuit passed embd=%s chat=%s rerank=%s total_elapsed=%.3fs",
                embd_passed,
                chat_passed,
                rerank_passed,
                time.perf_counter() - t_req,
    ...
    if verify_flag:
        success = len(msg.strip()) == 0
        logging.info(
            "[set_api_key] return VERIFY success=%s msg_len=%s total_elapsed=%.3fs",
            success,
            len(msg),
            time.perf_counter() - t_req,
        )
        return get_json_result(data={"message": msg, "success": success})

    if msg:
        logging.warning(
            "[set_api_key] return ERROR total_elapsed=%.3fs msg_len=%s",
            time.perf_counter() - t_req,
            len(msg),
        )
        return get_data_error_result(message=msg)
    ...
    logging.info(
        "[set_api_key] return SAVE_OK tenant=%s factory=%s rows=%s total_elapsed=%.3fs",
        current_user.id,
        factory,
        len(source_llms),
        time.perf_counter() - t_req,
...
async def add_llm():
    ...
    timeout_seconds = _probe_timeout_seconds(verify=req.get("verify", False))
    ...
    if factory == "VolcEngine":
        ...
        api_key_payload = req.get("api_key")
        if not isinstance(api_key_payload, dict):
            api_key_payload = {}
        token = req.get("mineru_token") or req.get("MINERU_TOKEN")
        if token and str(token).strip():
            if not str(api_key_payload.get("mineru_token", "")).strip():
                api_key_payload = {**api_key_payload, "mineru_token": str(token).strip()}
        req["api_key"] = api_key_payload
        api_key = apikey_json(["api_key", "provider_order"])
            ...
            try:
                async def check_streamly():
                    async for chunk in mdl.async_chat_streamly(
                        ...
                        if _probe_stream_chunk_ok(chunk):
                            return True
                    return False
