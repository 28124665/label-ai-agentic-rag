...
from common.constants import MessageTypeEnum
...
@manager.route("/oss_upload", methods=["POST"])  # noqa: F821
@validate_request("kb_id", "user_id", "source_oss_url")
async def oss_upload():
    form = await request.form
    kb_id = form.get("kb_id")
    user_id = form.get("user_id")
    source_oss_url = form.get("source_oss_url")
    ...
    logging.info(f"1.document_app.oss_upload kb_id({kb_id}), user_id({user_id}), source_oss_url({source_oss_url}), message_type({message_type})")

    e, kb = KnowledgebaseService.get_by_id(kb_id)
    if not e:
        raise LookupError("Can't find this dataset!")

    err, files = await thread_pool_exec(FileService.upload_oss_document, kb, source_oss_url, user_id, message_type)
    if err:
        files = [f[0] for f in files] if files else []
        return get_json_result(data=files, message="\n".join(err), code=RetCode.SERVER_ERROR)
...
@manager.route("/run_oss_document", methods=["POST"])  # noqa: F821
@validate_request("doc_ids", "run","uid")
async def run_oss_document():
    req = await get_request_json()
    uid = req.get("uid")
...
@manager.route("/oss_upload_and_run", methods=["POST"])  # noqa: F821
@validate_request("kb_id", "user_id", "source_oss_url", "message_type")
async def oss_upload_and_run():
    req = await get_request_json()
    kb_id = req.get("kb_id")
    ...
    logging.info(
        f"1.document_app.oss_upload_and_run kb_id({kb_id}), user_id({user_id}), source_oss_url({source_oss_url}), message_type({message_type})")
    ...
    doc_ids = [f[0]["id"] for f in files]   # remove the blob
    logging.info(f"oss_upload_and_run.doc_ids: {doc_ids}, user_id: {user_id}")
    ...
    try:
        def _run_sync():
            ...
            for doc_id in doc_ids:
                ...
                info = {"run": TaskStatus.RUNNING.value, "progress": 0}
                info["progress_msg"] = ""
                info["chunk_num"] = 0
                info["token_num"] = 0
            ...
            return get_json_result(data={"doc_ids": doc_ids})
